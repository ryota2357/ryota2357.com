...
export const allBlogCollection = await getCollection(
  "blog",
  ({ data }) =>
    data.vrt !== true && (import.meta.env.PROD ? data.draft !== true : true),
);
export const vrtBlogCollection = await getCollection(
  "blog",
  ({ data }) => data.vrt === true,
);
